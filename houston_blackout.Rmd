---
title: 'A spatial Analysis of Houston Blackouts'
...
across the United States on February 10--11, 13--17, and 15--20."[^1]

[^1]: Wikipedia. 2021. "2021 Texas power crisis." Last modified October
    2, 2021. <https://en.wikipedia.org/wiki/2021_Texas_power_crisis>.

The goal of this analysis is to find and estimate the total number of houses in Houston that lost power from the first two stomrs that hit in 2021. Specifically, I am investigating if socioeconomic factors are important predictors of a communitie's recovery following a power outage.
...
from the Suomi satellite. I used the VNP46A1 tolook for variation in night lights before and after the storm as a way ofidentifying areas that lost electric power as a result of the storms. Furthermore, data containing the physical perimiters of buildings and roads is needed to spatially join to areas that lost power. I gathered this data from [OpenStreetMap](https://www.openstreetmap.org/#map=4/38.01/-95.84) providning high resolution data on this. Finally, we need to join this data with socioeconomic factor data that influenced recovery,
so I pulled relating data from the US Census Bureau (see `Readme` file).

Some of the most important tools in carryign out this analysis incldue:
...
Using NASA's Worldview to find data pre and post storm, I found that satellite images on 
2021-02-07 and 2021-02-16 provided two clear, contrasting images to
visualize the extent of the power outage in Texas.
VIIRS data is distributed through NASA's [Level-1 and Atmospheric
Archive & Distribution System Distributed Active Archive Center (LAADS
DAAC)](https://ladsweb.modaps.eosdis.nasa.gov/). Many NASA Earth data
...
h08v06, and so I downloaded two tiles per date.
...
Highways make up much of the night lights seen from space (see Google's [Earth at
Night](https://earth.google.com/web/@27.44405464,-84.7693044,206.63660162a,8916361.52264659d,35y,0h,0t,0r/data=CiQSIhIgMGY3ZTJkYzdlOGExMTFlNjk5MGQ2ZjgxOGQ2OWE2ZTc)).
...
I vectorized the the blackout mask and fixed any invalid geometries using `st_make_valid`
...
of `st_read`'s ability to subset using a SQL query.

First I defined a SQL query: `query <- "SELECT * FROM gis_osm_roads_free_1 WHERE fclass='motorway'"`
 which I applied to `st_read` to load just highway data from geopackage. I then reprojected the data to EPSG:3083 and identified areas within 200m of all highways using `st_buffer` and dissolved them with `st_union`. Laslty, I find areas that experienced blackouts that are further than 200m
from a highway through sspatial subsetting using the `st_disjoint` operation.
...
Buildings dataset was loaded in using `st_read` and the following SQL query:
...
I then filtered to homes within blackout areas and counted he number of impacted homes.
...
Next, I joined the income data to the census tract geometries uaing a `left_join` function by "ID".
This was then spatially joined to the census tract data with buildings determined to be impacted by blackouts using the `st_intersects` operation. Lastly, census tracts that had blackouts were identified.
...
Finally, I created a map of median income by census tract and designated which tracts experienced blackouts. I then plot the distribution of income in impacted and unimpacted tracts to visualize what the spread of the communnity income looked like.
